...
!     
  ...
  sideh = 0.5d0*side
  rcoffs = rcoff*rcoff
  !$omp parallel do reduction(+:vir,epot) shared(npart,f,side,sideh,x,rcoff,rcoffs) private(rd, rrd, rrd2, rrd3, rrd4, rrd6, rrd7, r148, forcex, forcey, forcez,xi, yi, zi, fxi, fyi, fzi, xx, yy, zz ) default(none)
  do i = 1,npart
     xi = x(i,1)
     yi = x(i,2)
     zi = x(i,3)
